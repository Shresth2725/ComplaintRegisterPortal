        ...
        <div className="flex flex-col h-full bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                ...
                {messages.length === 0 ? (
                    ...
                    messages.map((msg, index) => {
                        ...
                        return (
                            ...
                            >
                                {/* Sender name */}
                                <span className={`text-xs mb-1 font-medium ${isOwnMessage ? "text-blue-600" : "text-slate-500"}`}>
            ...
            >
                <input
                    ...
                    className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                />
                <button
                    type="submit"
                    disabled={!inputMessage.trim()}
                    className="px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed rounded-lg text-white font-medium transition-colors shadow-sm"
                >
                    Send
                </button>
            </form>
